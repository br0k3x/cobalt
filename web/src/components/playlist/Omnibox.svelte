...
{#if env.DEFAULT_API !== officialApiURL}
    <div id="instance-label">
        {$t("playlist.label.community_instance")}
    </div>
{/if}
...
<div id="omnibox">
    ...
    >
        ...
        <input
            ...
            autocapitalize="off"
            maxlength="512"
            placeholder={$t("playlist.input.placeholder")}
    ...
    <div id="action-container">
        ...
        <ActionButton id="paste" click={pasteClipboard}>
            <IconClipboard />
            <span id="paste-desktop-text">{$t("playlist.paste")}</span>
            <span id="paste-mobile-text">{$t("playlist.paste.long")}</span>
        </ActionButton>
    </div>
</div>
